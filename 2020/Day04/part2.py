...
def check_field(field):
    ...
    check_method = {
        ...
        "hgt": lambda x: (x.endswith("in") and 76 >= int(x[:-2]) >= 59)
        or (x.endswith("cm") and 193 >= int(x[:-2]) >= 150),
...
def part2():
    ...
    for input in input_list:
        fields = sorted(input.split())
        if all([x in input for x in VALID_FIELDS]) and all(
            [check_field(field) for field in fields]
        ):
            valid_count += 1
